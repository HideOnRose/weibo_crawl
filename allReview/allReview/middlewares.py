...
class SeleniumMiddleware(object):
    ...
    def process_request(self, request, spider):
        ...
        for i in range(5):
            try:
                loading = self.browser.find_element_by_xpath("//div[@node-type='comment_loading']")
                self.browser.execute_script("arguments[0].scrollIntoView();", loading)
            except:
                break

        for i in range(20):
            try:
                btn = self.browser.find_element_by_xpath("//a[@action-type='click_more_comment']/span")  # 更多按钮
                btn.click()
                time.sleep(1)
            except:
                break
